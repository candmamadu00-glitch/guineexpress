...
// ==========================================
// FUNÇÃO: EXCLUIR CONTA DO CLIENTE
// ==========================================
async function excluirMinhaConta() {
    // Primeira confirmação padrão
    const confirmacao1 = confirm("⚠️ ATENÇÃO: Tem certeza que deseja excluir sua conta na Guineexpress?");
    if (!confirmacao1) return;

    // Segunda confirmação de segurança para evitar cliques acidentais
    const confirmacao2 = prompt("Esta ação é IRREVERSÍVEL e apagará seu acesso.\n\nPara continuar, digite a palavra: EXCLUIR");
    if (confirmacao2 !== "EXCLUIR") {
        alert("Exclusão cancelada pela segurança.");
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            // Redireciona o cliente para a tela de login/index
...
async function updateOrderWithProof(id, status, location, proofBase64) {
    ...
    try {
        ...
        if(data.success) {
            ...
            // 👉 A MÁGICA DA VELOCIDADE: Remove a encomenda dos dados locais se o status final for Entregue
            if (status === 'Entregue' && window.todasEncomendas) {
                const toggleBtn = document.getElementById('toggle-orders');
                // Só remove se o botão de "Mostrar Entregues" NÃO estiver marcado
                if (!toggleBtn || !toggleBtn.checked) {
                    window.todasEncomendas = window.todasEncomendas.filter(o => o.id !== id);
                }
            }
            
            loadOrders(); // Renderiza a tabela instantaneamente mais leve
        } else {
            alert("Erro: " + data.msg);
        }
    } catch(err) {
...
// ==========================================
// FUNÇÃO PARA O CLIENTE/ADMIN VER A FOTO DA ENTREGA (LEVE)
// ==========================================
async function viewDeliveryPhoto(code) {
    try {
        // Busca a imagem no servidor apenas quando o usuário clica no botão
        const response = await fetch(`/api/orders/${code}/photo`);
        const data = await response.json();

        if (!response.ok || !data.proof_image) {
            alert("❌ Foto não encontrada para esta encomenda.");
            return;
        }

        // Cria a tela preta transparente por cima de tudo para mostrar a foto
        ...
        modal.innerHTML = `
            <img src="${data.proof_image}" style="max-width:90%; max-height:80%; border-radius:10px; border:3px solid #009ee3; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
        ...
        alert("❌ Erro de conexão ao buscar a foto do comprovante.");
    }
}

// ==========================================
// ABA DE ENTREGAS INTELIGENTE (OTIMIZADA)
// ==========================================
async function loadDeliveryList() {
    try {
        ...
        // Tenta usar a memória local primeiro. Se não existir, aí sim busca no banco.
        let orders = window.todasEncomendas;
        if (!orders) {
            const response = await fetch('/api/orders'); 
            orders = await response.json();
            window.todasEncomendas = orders; // Salva para a próxima vez
        }
        ...
        // Atualiza o Filtro de Lotes de forma eficiente
        if (filterSelect) {
            const todosOsLotes = orders.map(o => o.lote || 'Sem Lote');
            const lotesUnicos = [...new Set(todosOsLotes)].sort();
        ...
        // Variável para acumular o HTML (Evita travar o celular com appendChild)
        let htmlTabela = '';
                        ...
                        ${isDelivered 
                            ? `<span style="color: #27ae60; font-weight: bold;"><i class="fas fa-check-circle"></i> JÁ ENTREGUE</span>
                               <div style="display:flex; justify-content:center; gap:5px; margin-top:8px;">
                                   ${order.has_proof_image ? `<button onclick='viewDeliveryPhoto("${order.code}")' style="color:#6f42c1; border:1px solid #6f42c1; background:none; padding:4px 10px; border-radius:4px; cursor:pointer; font-size:12px; font-weight:bold;" title="Ver foto enviada"><i class="fas fa-camera"></i> Foto</button>` : ''}
        ...
        // Injeta tudo de uma vez só na tela
        list.innerHTML = htmlTabela;
...
async function submitDelivery() {
    ...
    try {
        const response = await fetch(`/api/orders/${pendingDeliveryCode}/deliver`, { 
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ proofImage: currentProofBase64 }) 
        });

        if (response.ok) {
            alert("✅ Encomenda entregue! A foto foi salva e enviada automaticamente.");
            closeDeliveryModal();
            
            // 👉 A MÁGICA DA VELOCIDADE: Remove a encomenda dos dados locais na hora
            if (window.todasEncomendas) {
                window.todasEncomendas = window.todasEncomendas.filter(o => o.code !== pendingDeliveryCode);
            }
            
            // Recarrega a tabela já sem o item pesado
            loadDeliveryList(); 
        } else {
