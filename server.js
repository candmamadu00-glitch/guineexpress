...
app.use(compression({
    filter: (req, res) => {
        ...
        // Não tenta comprimir vídeos ou imagens para não gastar CPU do Render
        if (req.url.includes('/uploads/')) return false; 
...
// ==========================================
// 🛡️ PROTEÇÃO CONTRA ATAQUES (RATE LIMITERS)
// ==========================================
...
const authLimiter = rateLimit({
    windowMs: 60 * 60 * 1000,
    max: 10,
    message: { success: false, msg: "🚫 Muitas tentativas de acesso. Tente novamente em 1 hora." }
});
...
// Configuração de Notificações Push com fallback seguro
const vapidEmail = process.env.VAPID_EMAIL || process.env.EMAIL_USER || 'comercialguineexpress245@gmail.com';
...
async function enviarReciboPDF(invoiceId) {
    ...
    db.get(sql, [invoiceId], async (err, fatura) => {
        ...
        const isPaid = (statusStr === 'pago' || statusStr === 'paid' || statusStr === 'approved' || fatura.is_paid == 1);
...
const uploadsFolder = path.join(discoPermanente, 'uploads');
...
// MULTER
// ==========================================
// 📁 CONTROLE RÍGIDO DE UPLOADS (MULTER)
// ==========================================
...
// Limite para imagens/documentos: 5MB
const upload = multer({ 
    storage: storage,
...
// Limite para vídeos: 30MB (Mais do que isso o WhatsApp e o Render sofrem)
const uploadVideo = multer({ 
    ...
    fileFilter: (req, file, cb) => {
        if (file.mimetype.startsWith('video/')) {
            ...
            cb(new Error('Apenas arquivos de vídeo são permitidos nesta rota!'), false);
        }
    }
});

// Rate limit exclusivo para uploads pesados (Evita ataque DDoS enchendo seu HD)
const uploadLimiter = rateLimit({
    windowMs: 60 * 60 * 1000, // 1 hora
    max: 15, // Máximo de 15 vídeos por hora por IP
...
// BACKUP AUTOMÁTICO DIÁRIO (CORRIGIDO PARA O DISCO PERMANENTE)
cron.schedule('0 3 * * *', async () => {
    ...
    const mailOptions = {
        ...
        `,
        attachments: [{ filename: `backup_guineexpress_${dataAtual}.db`, path: dbPath }]
    };

    try {
        await transporter.sendMail(mailOptions);
        console.log(`✅ Backup enviado com sucesso para ${adminEmail}`);
    } catch (error) {
        console.error("❌ Erro no backup:", error);
    }
});

// IA GEMINI UTILITÁRIOS
...
function logSystemAction(req, action, details) {
    const userLogado = req.session ? req.session.user : null;
    ...
    db.run("INSERT INTO system_logs (user_name, action, details, ip_address) VALUES (?, ?, ?, ?)", 
        ...
        (err) => { if(err) console.error("Erro ao salvar log no sistema:", err); }
    );
}
...
// SESSÕES (CONFIGURAÇÃO PARA RENDER)
app.set('trust proxy', 1);

app.use(session({
    store: new SQLiteStore({
        db: 'guineexpress_v4.db',
        dir: discoPermanente // 🚀 Corrigido para a variável definida no topo
    }),
    secret: process.env.SESSION_SECRET || 'chave_ultra_secreta_guine_express_2026',
    resave: false,
    saveUninitialized: false,
    name: 'guine_sessid', // Nome genérico para esconder que é Express
    cookie: {
        httpOnly: true, // Impede que scripts maliciosos (XSS) leiam o cookie de sessão
        secure: process.env.NODE_ENV === 'production', // Requer HTTPS no Render
...
const employeeOnly = (req, res, next) => {
    if (req.session.userId && (req.session.role === 'employee' || req.session.role === 'admin')) return next();
    res.redirect('/login.html?error=acesso_negado');
};
...
// ==========================================
// ROTA NOVA: BUSCA A FOTO APENAS SE CLICAR NO BOTÃO
// ==========================================
app.get('/api/orders/:code/photo', (req, res) => {
    const orderCode = req.params.code;
    const sql = `SELECT proof_image FROM orders WHERE code = ?`;
    
    db.get(sql, [orderCode], (err, row) => {
        if (err || !row || !row.proof_image) {
            return res.status(404).json({ error: "Foto não encontrada" });
        }
        res.json({ proof_image: row.proof_image });
    });
});
// ==========================================
...
// ==========================================
// ROTA: EXCLUIR PRÓPRIA CONTA (Apenas Cliente)
// ==========================================
app.delete('/api/client/me', (req, res) => {
    // 1. Verifica se quem está acessando é realmente um cliente logado
    if (!req.session.userId || req.session.role !== 'client') {
        return res.status(403).json({ success: false, msg: 'Acesso negado.' });
    }
    ...
    // 2. Apaga o usuário do banco de dados usando o ID da sessão
    db.run("DELETE FROM users WHERE id = ? AND role = 'client'", [clientId], function(err) {
        if (err) {
            console.error("Erro ao excluir conta do cliente:", err);
            return res.status(500).json({ success: false, msg: 'Erro interno ao excluir conta.' });
        }
        
        if (this.changes > 0) {
            // Registra a ação no sistema e destrói a sessão (desloga o usuário)
            if (typeof logSystemAction === 'function') {
                logSystemAction(req, "EXCLUIR CONTA", `O cliente ID: ${clientId} excluiu a própria conta.`);
            }
            req.session.destroy();
            res.json({ success: true, msg: 'Conta excluída com sucesso!' });
        } else {
            res.status(404).json({ success: false, msg: 'Conta não encontrada ou já excluída.' });
        }
    });
});
// ==============================================================
// 🌟 ROTA DE ATUALIZAÇÃO EM MASSA DE STATUS (INTEGRADA COM ZAP QUEUE)
// ==============================================================
app.post('/api/orders/bulk-update-status', (req, res) => {
    ...
    db.run(sqlUpdate, [status, ...ids], function(err) {
        ...
        const updatedCount = this.changes;
        console.log(`\n✅ [AÇÃO EM MASSA] ${updatedCount} encomendas alteradas para '${status}'`);

        // Responde rápido para a interface do painel
        res.json({ success: true, updated: updatedCount });

        // Busca os clientes afetados para notificar
        const sqlSelect = `
            SELECT o.code, o.description, u.id as client_id, u.name, u.phone 
            FROM orders o
            JOIN users u ON o.client_id = u.id
        ...
        db.all(sqlSelect, ids, (err, rows) => {
            if (err || !rows) return console.error("Erro ao buscar clientes em massa:", err);

            for (const row of rows) {
                const desc = row.description ? row.description : 'Sua encomenda';

                // 1. Notificação interna do painel
                const tituloAviso = "Atualização de Encomenda";
                const msgAviso = `Sua encomenda ${row.code} mudou para: ${status}`;
                db.run("INSERT INTO notifications (user_id, title, message, is_read) VALUES (?, ?, ?, 0)", 
                    [row.client_id, tituloAviso, msgAviso]);
                ...
                // 2. Coloca a mensagem na Fila do WhatsApp
                if (row.phone) {
                    let cleanPhone = row.phone.replace(/\D/g, '');
                    if (cleanPhone.length === 10 || cleanPhone.length === 11) {
                        cleanPhone = '55' + cleanPhone;
            ...
            console.log(`🚀 [AÇÃO EM MASSA] ${rows.length} mensagens inseridas na fila do WhatsApp com sucesso!`);
        });
    });
});
// ==========================================
...
// ==============================================================
// 🚀 INICIANDO O SERVIDOR E O ZAP AUTOMATICAMENTE
// ==============================================================
...
app.listen(PORT, () => {
    ...
    // Liga o motor do Zap automaticamente 2 segundos após o servidor iniciar
    setTimeout(() => {
        ligarMotorDoZap();
    }, 2000);
});